    ...
    while True: # game loop
        ...
        if time.time() - lastFallTime > fallFreq:
            ...
            if not isValidPosition(board, fallingPiece, adjY=1):
                ...
                numLinesRemovedThisTurn = removeCompleteLines(board) # Store num lines removed
                score += numLinesRemovedThisTurn # Original score update
                totalLinesCleared += numLinesRemovedThisTurn
                level, fallFreq = calculateLevelAndFallFreq(score)
                fallingPiece = None
            else:
        ...
        DISPLAYSURF.fill(BGCOLOR)
        drawBoard(board)
        drawStatus(score, level, totalLinesCleared)
...
def drawStatus(score, level, linesCleared):
    ...
    linesSurf = BASICFONT.render('Lines: %s' % linesCleared, True, TEXTCOLOR)
